...
By specifying `vc templates --help` you can also view other VC template commands (besides import you can list, export
and remove)

## REST API

### List templates you have - GET /issuer-api/{tenantId}/config/templates
...
### Show a VC template - GET /issuer-api/{tenantId}/config/templates/{templateId}
...
### Add a new VC template - POST /issuer-api/{tenantId}/config/templates/{templateId}

#### VC template

For this demo, we will add the example w3c-ccg
credential [CrudeProductCredential](https://w3c-ccg.github.io/vc-examples/crude/examples/v1.0/crude-product-verifiable-credential-v1.0.json):
...
curl -X 'POST' \
  ...
  -d 'JSON credential seen above'
```

### Remove the VC template again - /issuer-api/{tenantId}/config/templates/{templateId}
